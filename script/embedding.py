...
class ComplEx(torch.nn.Module):
    def __init__(self, path, nbfentmap, nbfrelmap, div=True):
        ...
        r_dim = int(ckpt['embeddings.1.weight'].size(1)/2)
        e_dim = int(ckpt['embeddings.0.weight'].size(1)/2)
        embs = {}
        embs['E_im.weight'] = ckpt['embeddings.0.weight'][:22021, e_dim:]
        embs['E_re.weight'] = ckpt['embeddings.0.weight'][:22021, :e_dim]
        embs['R_im.weight'] = ckpt['embeddings.1.weight'][:, r_dim:]
        embs['R_re.weight'] = ckpt['embeddings.1.weight'][:, :r_dim]
        r_map = nbfrelmap
        e_map = nbfentmap
...
class KDG(torch.nn.Module):
    def __init__(self, cfg, device,  nbfentmap, nbfrelmap):
        super(KDG, self).__init__()
        self.path = cfg.kdg
        ckpt_path = os.path.join(self.path, f"{cfg.dataset.fold}model.pt")
        self.model = torch.load(ckpt_path).to(device)
        self.model.requires_grad_(False)
        self.lhs = self.model.lhs(torch.tensor([i for i in range(len(nbfentmap))]).to(device))
        self.rhs = self.model.rhs(torch.tensor([i for i in range(len(nbfentmap))]).to(device))
        self.rel = self.model.rel.to(device)
        self.gatecell = self.model.gatecell
        self.gate = self.model.gate
        self.num_entities = len(self.rhs)
        self.num_genes = 8947
        self.num_disgenes = 22021

    def get_score(self, h, r):
        lhs = self.lhs[h]
        rhs = self.rhs[:self.num_disgenes]
        rel = self.rel(r)
        result = torch.zeros(h.size(0), self.num_disgenes).to(h.device)
        for i in range(len(h)):
            lhs_ = torch.unsqueeze(lhs[i], dim=0)
            lhs_ = torch.repeat_interleave(lhs_, self.num_disgenes, dim=0)
            rel_ = torch.unsqueeze(rel[i], dim=0)
            rel_ = torch.repeat_interleave(rel_, self.num_disgenes, dim=0)
            if self.gatecell == 'LSTMCell':
                c = torch.zeros_like(lhs_)
                rel_update, c1 = self.gate(rel_, (lhs_, c))
            else:
                rel_update = self.gate(rel_, lhs)

            result[i] = torch.squeeze(torch.sum(lhs_ * rel_update * rhs, 1, keepdim=True))
...
class DistMult(torch.nn.Module):
    def __init__(self, cfg, device):
        ...
        self.num_genes = 8947
        self.num_disgenes = 22021
        self.path = cfg.distmult
        ckpt_path = os.path.join(self.path, f"{cfg.dataset.fold}model.pt")
        self.model = torch.load(ckpt_path).to(device)
        self.model.requires_grad_(False)
        self.e_emb = self.model.embeddings[0](torch.tensor([i for i in range(self.num_disgenes)]).to(device))
        self.r_emb = self.model.embeddings[1]
        self.num_entities = len(self.e_emb)


    def get_score(self, h, r):
        lhs = self.e_emb[h]
        rhs = self.e_emb[:self.num_disgenes]
        rel = self.r_emb(r)
        result = torch.zeros(h.size(0), self.num_disgenes).to(h.device)
        for i in range(len(h)):
            lhs_ = torch.unsqueeze(lhs[i], dim=0)
            ...
            result[i] = torch.squeeze(torch.sum(lhs_ * rel_ * rhs, 1, keepdim=True))
...
class SimplE(torch.nn.Module):
    def __init__(self, cfg, device):
        ...
        self.num_genes = 8947
        self.num_disgenes = 22021
        self.path = cfg.simple
        ckpt_path = os.path.join(self.path, f"{cfg.dataset.fold}model.pt")
        self.model = torch.load(ckpt_path).to(device)
        self.model.requires_grad_(False)
        self.t_emb = self.model.embeddings[3](torch.tensor([i for i in range(self.num_disgenes)]).to(device))
        self.h_emb = self.model.embeddings[2](torch.tensor([i for i in range(self.num_disgenes)]).to(device))
        self.r_emb = self.model.embeddings[0]
        self.ri_emb = self.model.embeddings[1]
        self.num_entities = len(self.t_emb)


    def get_score(self, h, r):
        lhs = self.h_emb[h]
        rhs = self.t_emb[:self.num_disgenes]
        rel = self.r_emb(r)
        reli = self.ri_emb(r)
        ...
        for i in range(len(h)):
            ...
            rel_ = torch.unsqueeze(rel[i], dim=0)
            rel_ = torch.repeat_interleave(rel_, self.num_disgenes, dim=0)
            reli_ = torch.unsqueeze(reli[i], dim=0)
            reli_ = torch.repeat_interleave(reli_, self.num_disgenes, dim=0)

            f = torch.squeeze(torch.sum(lhs_ * rel_ * rhs, 1, keepdim=True))
            b = torch.squeeze(torch.sum(rhs * reli_ * lhs_, 1, keepdim=True))
