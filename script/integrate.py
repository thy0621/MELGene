...
def get_WN_mappings(dataset, root, rotate = False):
    if (dataset == 'WN18RR'):
        ...
        NUM_RELATIONS = 7
    rnnrel = {}
    with open(PREFIX + 'rel_id', 'r') as fi:
        for line in fi:
            line = line.strip().split()
            rnnrel[line[0]] = int(line[1])
    rnnent = {}
    with open(PREFIX + 'ent_id', 'r') as fi:
        for line in fi:
            line = line.strip().split()
            rnnent[line[0]] = int(line[1])
    nbfent = {}
    idx = 0
    src = []
    dst = []
    ...
    for _ in dst:
        if _ not in nbfent:
            nbfent[_] = idx
            idx += 1
    if (WN == 0):#dataset == 'WN18RR'
        nbf2rnnent = {}
        for _ in nbfent:
            nbf2rnnent[nbfent[_]] = rnnent[_]
        nbf2rnnrel = {}
        for _ in nbfrel:#rel2id
